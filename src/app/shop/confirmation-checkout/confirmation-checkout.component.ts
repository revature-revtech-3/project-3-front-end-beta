...
export class ConfirmationCheckoutComponent implements OnInit {
  ...
  errorMsg: string = "";
  transId: any = 0;
  constructor(private activatedRoute: ActivatedRoute, private purchasedItemService: PurchasedItemService, private mailService: MailService) { }
  ...
  loadEmail() {
    this.mailService.email().subscribe((response) => {
